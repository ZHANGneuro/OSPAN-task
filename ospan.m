...
for num_of_clicks = 1:2
    while 1
        [clicks(num_of_clicks),x,y,whichButton] = GetClicks([],0.001);
        if clicks(num_of_clicks) ==1
            img = imread(['./image/' num2str(num_of_clicks+1) '.png']);
            [y, x, depth] = size(img);
            x = x/2.5;
            y = y/2.5;
            t1 = Screen('MakeTexture', wPtr, img);
...
for num_of_clicks = 1:2
    while 1
        [clicks(num_of_clicks),x,y,whichButton] = GetClicks([],0.001);
        if clicks(num_of_clicks) ==1
            img = imread(['./image/' num2str(num_of_clicks+4) '.png']);
            [y, x, depth] = size(img);
            x = x/2.5;
            y = y/2.5;
            t1 = Screen('MakeTexture', wPtr, img);
...
if clicks ==1
    ...
    for num_of_clicks = 1:2
        while 1
            [clicks(num_of_clicks),x,y,whichButton] = GetClicks([],0.001);
            if clicks(num_of_clicks) ==1
                img = imread(['./image/' num2str(num_of_clicks+7) '.png']);
                [y, x, depth] = size(img);
                x = x/2;
                y = y/2;
                t1 = Screen('MakeTexture', wPtr, img);
